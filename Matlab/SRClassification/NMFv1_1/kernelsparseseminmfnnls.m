function [AtA,Y,numIter,tElapsed,finalResidual]=kernelsparseseminmfnnls(X,k,option)
...
if ~strcmp(option.kernel,'linear')
    ...
    [~,Y]=kernelsparseseminmfnnls(X,k,optionIni);
else
    Y=rand(k,c);
end
% initialize AtA
...
% Y=rand(k,c);
prevRes=Inf;
% lambdaTlambda=option.lambda .* ones(k,k);
% iterative updating
for i=1:option.iter
    % update Y
    Y=kfcnnls2(AtA+option.lambda,AtX);
...
%     [AtA,~,XtX,~,AtX]=normalizeKernelMatrix(AtA,XtX,AtX);
    if mod(i,20)==0 || i==option.iter
        ...
        if option.tof>=fitRes || option.residual>=curRes || i==option.iter
            disp(['kernelsparseseminmfnnls successes!, # of iterations is ',num2str(i),'. The final residual is ',num2str(curRes)]);
            numIter=i;
