...
load('smallRoundBlueCellTumorOFChildhood.mat','Data','classes');
D=Data;
clear('Data');
kfold=3;
...
% optionMSRC.ks=6*ones(numUnikClass,1); % you can change the number of meta-sample by yourself.
...
% unikClass=unique(trainClass);
% numUnikClass=numel(unikClass);
% optionMNNLS.ks=6*ones(numUnikClass,1); % you can change the number of meta-sample by yourself.
% optionMNNLS.metaSampleMethod='nmf';
...
disp('SRC1 classifier for facial data...');
% downsample
...
% suppose the current folder is the one containing the SR Toolbox
load('breastTissue.mat','D','classes');
D=D';
...
load('InfBetaTensorComplete.mat','D','classes');
kfold=9;
ind=crossvalind('Kfold',classes,kfold);
indTest=(ind==1);
trainSet=D(:,:,~indTest);
